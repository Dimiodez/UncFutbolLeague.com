export async function ensureAdminOperations(env) {
  await env.DB.batch([
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS competition_lifecycle (
    ...
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS admin_messages (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      actor_discord_id TEXT NOT NULL REFERENCES users(discord_id),
      message TEXT NOT NULL,
      created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
    )`),
    env.DB.prepare('CREATE INDEX IF NOT EXISTS admin_messages_recent ON admin_messages(created_at DESC)')
  ]);
}
