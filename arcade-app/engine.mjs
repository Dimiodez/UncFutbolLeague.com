...
export const LEVELS = [
  ...
  {name:'Double trouble',goals:7,speed:620,goalWidth:84,goalSpeed:1.8,rows:2,hp:6,wall:false,bus:true,buses:2},
];
...
export function goalCenters(g){return g.level===12?[320+Math.sin(g.time*1.8)*205,320+Math.sin(g.time*2.1+1.6)*205]:[goalX(g)];}
...
export function awardGoal(g,count=1){g.saveBoost=0;g.score+=500*count;g.goals+=count;g.events.push('goal');if(g.goals>=LEVELS[g.level].goals){g.score+=250;g.phase=g.level===LEVELS.length-1?'won':'levelup';g.events.push(g.phase);}else resetBall(g);}
...
export function keeperX(g){if(g.level===12)return 320+Math.sin(g.time*4.8)*240;return clamp(goalX(g)+Math.sin(g.time*(2.5+(g.level-7)*.15))*65,50,590);}
...
export function update(g,dt,target){
  ...
  if(b.y<=40+BALL_R&&b.vy<0){const l=LEVELS[g.level];const nets=goalCenters(g).filter(x=>Math.abs(b.x-x)<l.goalWidth/2-BALL_R).length;if(nets){burst(g,b.x,b.y);if(nets===2)g.events.push('double-netter');awardGoal(g,nets);return;}b.y=40+BALL_R;b.vy=Math.abs(b.vy);}
