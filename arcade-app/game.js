...
function shoot(){if(g.phase!=='ready')return;const angle=Number($('#launch-angle').value);record(['launch',angle]);launch(g,angle);sync();canvas.focus({preventScroll:true});}
$('#shoot').addEventListener('click',shoot);
$('#launch-angle').addEventListener('input',()=>{const n=Number($('#launch-angle').value);$('#angle-value').textContent=Math.abs(n)+'° · '+(n<0?'Left':n>0?'Right':'Straight');});
...
canvas.addEventListener('pointerdown',e=>{e.preventDefault();canvas.focus({preventScroll:true});canvas.setPointerCapture(e.pointerId);const r=canvas.getBoundingClientRect();target=(e.clientX-r.left)*WIDTH/r.width;if(g.phase==='ready'){target=clamp(Math.round(target),0,640);record(['aim',target]);update(g,0,target);shoot();}});
...
function draw(){
  ...
  if(g.phase==='ready'){ctx.fillStyle='#e8ffca';ctx.textAlign='center';ctx.font='600 16px system-ui';ctx.fillText('SET ANGLE ABOVE · SPACE OR TAP TO SHOOT',320,530);const angle=Number($('#launch-angle').value)*Math.PI/180;ctx.save();ctx.setLineDash([7,6]);line(g.ball.x,g.ball.y-12,g.ball.x+Math.sin(angle)*130,g.ball.y-Math.cos(angle)*130,'#ffffffbb',3);ctx.restore();}
