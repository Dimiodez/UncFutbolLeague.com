...
function frame(now){const pad=Array.from(navigator.getGamepads?.()||[]).find(Boolean);const padLeft=pad?.buttons[14]?.pressed,padRight=pad?.buttons[15]?.pressed,padUp=pad?.buttons[12]?.pressed,padDown=pad?.buttons[13]?.pressed,padShoot=!!pad?.buttons[0]?.pressed;if(padShoot&&!padShootHeld)action();padShootHeld=padShoot;const elapsed=Math.min((now-last)/1000||0,0.05);last=now;accumulator+=elapsed;while(accumulator>=1/120){if(g.phase==='ready'){target=WIDTH/2;if(keys.has('arrowleft')||keys.has('a')||padLeft)aimAngle-=75/120;if(keys.has('arrowright')||keys.has('d')||padRight)aimAngle+=75/120;if(keys.has('arrowup')||keys.has('w')||padUp)aimAngle-=Math.sign(aimAngle)*Math.min(Math.abs(aimAngle),75/120);if(keys.has('arrowdown')||keys.has('s')||padDown)aimAngle+=(Math.sign(aimAngle)||1)*75/120;aimAngle=clamp(aimAngle,-60,60);}else{if(keys.has('arrowleft')||keys.has('a')||padLeft)target-=610/120;if(keys.has('arrowright')||keys.has('d')||padRight)target+=610/120;}target=clamp(target,paddleWidth(g)/2+8,WIDTH-paddleWidth(g)/2-8);target=Math.round(target);if(['playing','ready'].includes(g.phase))record([target,1]);update(g,1/120,target);accumulator-=1/120;}if(g.events.length){const meaningful=g.events.filter(e=>!['hit','bounce'].includes(e));if(meaningful.length)$('#announcement').textContent=meaningful.includes('goal')?'Goal!':meaningful.includes('miss')?'Ball lost. '+g.lives+' lives left.':meaningful.join(' ');g.events=[];sync();}draw();requestAnimationFrame(frame);}
