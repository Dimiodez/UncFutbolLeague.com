...
Loosey Goosey lives at /arcade/loosey-goosey, with static assets in goose-app/. The shared deterministic engine verifies fixed-step jump/glide inputs through /api/goose/run. The goose_bests table keeps one highest-scoring run per signed-in player, including distance, pace and honks; guests do not save scores. Tables are provisioned idempotently (migration 0010_goose.sql). Run node --test scripts/goose.test.mjs scripts/arcade.test.mjs for gameplay replay and persistence checks.

Sandy Uppy is the third game at `/arcade/sandy-uppy`, with a standalone React/Phaser build in `sandy-app/`. Source is the `codex/unc-arcade-sandy-uppy` branch of UncWheelUnited; build with `pnpm run build:sandy` and copy only `dist-sandy/assets` and `sandy.html` (as `index.html`). Its starter castle reappears on every fresh run. Signed-in completed runs save one best per member through `/api/sandy/run`; the public board shows score, survival time, and best combo. Guests do not persist scores in the published game. Tables provision idempotently on first use.

Sandy scoring is recomputed from bounded event logs with authenticated run tokens, origin checks, rate limits, wall-time checks, unique obstacle IDs, and atomic completion. Unlike Cleat and Goose, this first Phaser release does not replay collisions on the server; fabricated plausible events remain possible. This is casual competition, not cheat-proof ranking. Run `node --test scripts/sandy.test.mjs` for scoring and authenticated persistence checks.
